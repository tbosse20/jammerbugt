...
def load_images(super_folder, chunk_size, angle=0, save=False):
    ...
    folder_name = ConvertImages.handle_sub_folder(super_folder, 'images')

    # Load the TIF image
    images = []
    for i, file_path in enumerate(os.listdir(super_folder)):
        if not (file_path.endswith(".tif") or file_path.endswith("png")): continue
